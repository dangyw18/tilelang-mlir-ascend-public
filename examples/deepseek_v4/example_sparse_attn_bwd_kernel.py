...
def sparse_mqa_bwd_interface(q, kv, attn_sink, o, do, topk_idxs, lse, sm_scale):
    ...
    # Postprocess dkv to convert from fp32 to bf16 with reduced register pressure.
...
def run_one_bwd_case(
    ...
    torch.testing.assert_close(tl_d_attn_sink.float(), ref["d_attn_sink"].float(), rtol=5e-2, atol=5e-2)
    print("\033[92mattn_sink check passed.\033[0m")
    torch.testing.assert_close(tl_dkv.to(torch.bfloat16).float(),ref["dkv"].to(torch.bfloat16).float(), rtol=5e-2, atol=5e-2)
